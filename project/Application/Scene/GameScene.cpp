...
void GameScene::Update([[maybe_unused]] GameManager *Scene)
{
  ...
  ImGui::DragFloat("scale::%f", &PostEffect::GetInstance()->GetAdjustedColorParam().fogScale_,
                   0.01f);
  ImGui::DragFloat("att::%f",
                   &PostEffect::GetInstance()->GetAdjustedColorParam().fogAttenuationRate_, 0.01f);
...
void GameScene::Gravitys()
{
  ...
  gravityManager_->PushParticleList(
      lavaManager_->GetLava(0).lock()->GetLavaParticle()->GetParticle());
