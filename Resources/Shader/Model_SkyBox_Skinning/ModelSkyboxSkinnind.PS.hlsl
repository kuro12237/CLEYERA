...
TextureCube<float32_t4> gSkyBox : register(t4);
...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    N = normalize(N);
    //環境マップ
    float32_t3 reflectV = reflect(toEye, N);
    float32_t4 envroinmentColor = gSkyBox.Sample(gSampler, reflectV);
    ...
    resultColor.rgb = envroinmentColor.rgb;
